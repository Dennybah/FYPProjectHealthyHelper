package com.example.healthyHelper.recipes;
...
public class CrunchyBlueberryYogurtRecipe extends AppCompatActivity {
    ...
    public static final String blueberryYogurtRecipeState = "blueberryYogurtRecipeState";

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.recipe_crunchy_blueberry_yogurt);

        //Adds custom toolbar to app screen
        Toolbar myToolbar = findViewById(R.id.toolBar);
        setSupportActionBar(myToolbar);
        Objects.requireNonNull(getSupportActionBar()).setTitle("Blueberry Yogurt Recipe");

        hyperLink = findViewById(R.id.hyperlink);
        addToCart=findViewById(R.id.shoppingCartButton);
    ...
    //Controls functionality for adding the recipe to the shopping cart, writing if it is added or not to shared preferences
    private void addToCartButton(){
        ...
        SharedPreferences.Editor editor = blueberryYogurtStoredState.edit();

        addToCart.setOnClickListener(view -> {
            if (!blueberryYogurtStoredState.getBoolean("added", false)) {
                Toast.makeText(getBaseContext(), "Added ingredients to shopping cart, Hold the button to remove them", Toast.LENGTH_SHORT).show();
                editor.putBoolean("added", true).apply();
            }
        });
        addToCart.setOnLongClickListener(view -> {
            if (blueberryYogurtStoredState.getBoolean("added", false)) {
                Toast.makeText(getBaseContext(), "Removed ingredients from shopping cart", Toast.LENGTH_SHORT).show();
    ...
    private void zoomText(MotionEvent motionEvent) {
        if(motionEvent.getPointerCount()==2) {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            case (R.id.articlesMenuItem):
                intent = new Intent(CrunchyBlueberryYogurtRecipe.this, ArticlesMenu.class);
                startActivity(intent);
                return true;
            case (R.id.bloodRecorderMenuItem):
                intent = new Intent(CrunchyBlueberryYogurtRecipe.this, BloodGlucoseRecorderMenu.class);
                startActivity(intent);
                return true;
            case (R.id.recipeMenuItem):
                intent = new Intent(CrunchyBlueberryYogurtRecipe.this, RecipesMenu.class);
                startActivity(intent);
                return true;
            case (R.id.shoppingCartMenuItem):
                intent = new Intent(CrunchyBlueberryYogurtRecipe.this, ShoppingCartMenu.class);
                startActivity(intent);
                return true;
            case (R.id.homeMenuItem):
                intent = new Intent(CrunchyBlueberryYogurtRecipe.this, MainMenu.class);
                startActivity(intent);
                return true;
            case (R.id.howToUse):
                intent = new Intent(CrunchyBlueberryYogurtRecipe.this, HowToUseFeatures.class);
                startActivity(intent);
                return true;
            default:
